      ...
      <div class="row">
		...
		<?php $checkPermission=Yii::app()->session['permissions'];
			 if($checkPermission[0]['lists']==0){
				 echo CHtml::link('Pin List <i class="fa clip-list-6"></i>',array('pinmaster/index'), array('class'=>"btn btn-orange add-row"));
			}?>
        </div>
        <div class="col-md-12"> 
              ...
              <div class="modal-body">
                <?php $form=$this->beginWidget('CActiveForm', array(
                    ...
                    <div class="col-md-12">
                     ...
                     <?php echo $form->labelEx($model,'pkg_id',array('class'=>"control-label", 'for'=>"")); ?>
                      <?php 
						     $package_name = CHtml::listData(Packages::model()->findAll(array("condition"=>"status =  '0'","order"=>"package_name")), 'id', 'package_name');
							echo $form->dropDownList($model, 'pkg_id', $package_name, array('id'=>'pkg_id','prompt' => 'Select package','class'=>"form-control",'id'=>"pkg_id"));?>
                             <?php echo $form->error($model,'pkg_id'); ?>	
                    </div>
                         
                         
                        <div class="form-group">
                           <?php echo $form->labelEx($model,'transfer_to',array('class'=>"control-label", 'for'=>"")); ?>
                           <?php echo $form->textField($model,'transfer_to',array('id'=>'transfer_to', 'class'=>"form-control",'onchange'=>"sponser(this.value); sponsorname(this.value);")); ?>
                            <span id="user_name" style="text-align:right; float:right; color:red;"></span>
                           <?php echo $form->error($model,'transfer_to'); ?> </div>
                        <div class="form-group">
                           <?php echo $form->labelEx($model,'no_pin',array('class'=>"control-label", 'for'=>"")); ?>
                           <?php echo $form->textField($model,'no_pin',array('size'=>10,'id'=>'no_pin', 'class'=>"form-control")); ?>
                           <?php echo $form->error($model,'no_pin'); ?> </div>
...
<script>
       ...
       // alert('function is call');return false;
		 var sponser_id=$('#transfer_to').val();
		 jQuery.ajax({
